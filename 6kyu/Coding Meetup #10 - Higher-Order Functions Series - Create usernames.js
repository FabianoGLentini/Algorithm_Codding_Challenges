...
function addUsername(list) {
    const fN = list.map( dev => (dev.firstName).toLowerCase())
    const lN = list.map( dev => dev.lastName[0].toLowerCase())
    const bYear = list.map( dev => (new Date().getFullYear()) - dev.age)
    
    
    list.forEach( (dev, i) => dev.username = (fN[i]+lN[i]+bYear[i]))
