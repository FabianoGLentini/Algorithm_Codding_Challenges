...
const snap = (f, t, i=0, c = [0,1]) => {
    // check if turtle length reached 0, if true we return up our recusion the value of 0
    if (!t.length) return 0    

    // we do the following code before cehcking the condition, when condition is met the loop ends
    do {
        // check if the turtle value has a valid value, if not we do same as the if statement
        if (!t[0]) return 0
        
        c.unshift((i%2?t:f).shift());
        i++;        
    } while (c[0]!=c[1]);    
    return 1 + snap([...f,...c.slice(0,-2).reverse()],t);    
}