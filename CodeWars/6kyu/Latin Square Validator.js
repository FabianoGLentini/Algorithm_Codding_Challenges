// Background
// A latin square is an n × n array filled with the integers 1 to n, each occurring once in each row and column.

// Here are examples of latin squares of size 4 and 7:

// [[1, 4, 3, 2],      [[2, 3, 1, 7, 4, 6, 5],
//  [4, 3, 2, 1],       [7, 1, 6, 5, 2, 4, 3],
//  [3, 2, 1, 4],       [6, 7, 5, 4, 1, 3, 2],
//  [2, 1, 4, 3]]       [4, 5, 3, 2, 6, 1, 7],
//                      [5, 6, 4, 3, 7, 2, 1],
//                      [1, 2, 7, 6, 3, 5, 4],
//                      [3, 4, 2, 1, 5, 7, 6]]
// Latin squares have many practical uses, for example in error-correcting-codes and the design of agricultural experiments. See https://en.wikipedia.org/wiki/Latin_square for more details. Sudoku is a 9 x 9 latin square, with additional conditions.

// Creating a latin square of any size is fairly straightforward (see Latin Squares). Validating that an arbitrary array is a latin square is slightly trickier, especially if you want to provide helpful feedback messages in case it's not.

// Task
// Write a function that validates if an input array is a latin square. It has two parameters, the array and a positive integer m > 1. To help the user with debugging, it should return an appropriate message, as detailed below. It must not modify the input array.

// Details
// The input array is guaranteed to be 2D and contain only integers. If it is a valid latin square of size m, the function should return "Valid latin square of size <m>". Otherwise, it should return one of the following messages:

// (1) If the input array is not square, the function should return "Array not square".

// (2) If the input array is square, but not of size m, the function should return "Array is wrong size". NOTE: When the array is both not square and the wrong size, (1) applies, so "Array not square" should be returned.

// (3) If any value between 1 and m in the array occurs more than once in a particular row, the function should identify it and its row by returning "<value> occurs more than once in row <s>". If there are multiple such values, only one should be identified. Row indexes run from 1 to m.

// (4) If any value between 1 and m in the array occurs more than once in a particular column, the function should return "<value> occurs more than once in column <t>". If there are multiple such values, only one should be identified. Column indexes run from 1 to m.

// (5) If any value in the array is not between 1 and m, the function should identify it and its location by returning "<value> at <row,col> is not between 1 and <m>". If there are multiple such values, only one should be identified.

// If more than one of the above conditions occur, the function should return one of the appropriate messages. For example, suppose m is 4 and the array is

// [[1, 2, 1, 1],
//  [5, 2, 3, 4],
//  [4, 3, 0, 2, 1],
//  [2, 1, 4, 3]].
// Then the function can return any of the following:

// "Array not square"

// "1 occurs more than once in row 1"

// "2 occurs more than once in column 2"

// "5 at 2,1 is not between 1 and 4"

// "0 at 3,3 is not between 1 and 4"

// Worth Noting
// There is no known easily-computable formula for the number of latin squares of a given size (see https://en.wikipedia.org/wiki/Latin_square - Number of n × n Latin squares).

// If you like this kata, you might also be interested in Sudokuboard validator.

// STRINGSARRAYS


//Code Wars Solution breakdown

function verifyLatinSquare(array, m) {
    //n will equal array length for easy use
    let n = array.length

    // tackles the (1) conditions if met return a specific string
    if(array.some(x => x.length != n)) return "Array not square"

    // tackles the (2) condition, if met return specific string
    if(n != m)                         return "Array is wrong size"
    

    //we are creating the row and colum variable for condition (3) and condition (4)
    for(let i = 0; i < n; i++){
      
        //we create the row and cols as set objects
      let rows = new Set(), cols = new Set()

      //we start our for loop within for loop to allow us to use [j] to find variable either on teh colum or row and place them in the appropriate variable rows or cols
      for(let j = 0; j < n; j++){

        //We are checking if the row wich is a Set object now has the indicate array value, it will return a true or false value and if it is true it means it happens more then once and it will return that number

        /*This works because rows and also cols will be empty until we add said  number to that set, so if rows or cols do have said number when checking that means it is indeed a duplicate. We dont need to worry about numbers staying for future loops as our first loop resets rows and cols */ 
        if(rows.has(array[i][j])) return `${array[i][j]} occurs more than once in row ${i + 1}`


        //we are adding said value to the row set 
        rows.add(array[i][j])

        //check if cols has a duplicate of said element same explination as rows above in the /**/, important we  are swapping[i][j] to [j][i]
        if(cols.has(array[j][i])) return `${array[j][i]} occurs more than once in column ${i + 1}`

        //repeat prior explination for the add method
        cols.add(array[j][i])

        //we check condition (5) and return appropriate string
        if(array[i][j] < 1 || array[i][j] > m) return `${array[i][j]} at ${i + 1},${j + 1} is not between 1 and ${m}`
      }
    }

    //if all the conditions arent met, congrats, its a valid latin square return appropriate string
    return `Valid latin square of size ${m}`
  }

//Code wars solution on its own
  
function verifyLatinSquare(array, m) {
    let n = array.length

    if(array.some(x => x.length != n)) return "Array not square"

    if(n != m)                         return "Array is wrong size"
    

    for(let i = 0; i < n; i++){
      
      let rows = new Set(), cols = new Set()

      for(let j = 0; j < n; j++){


        if(rows.has(array[i][j])) return `${array[i][j]} occurs more than once in row ${i + 1}`


        rows.add(array[i][j])

        if(cols.has(array[j][i])) return `${array[j][i]} occurs more than once in column ${i + 1}`

        cols.add(array[j][i])

        if(array[i][j] < 1 || array[i][j] > m) return `${array[i][j]} at ${i + 1},${j + 1} is not between 1 and ${m}`
      }
    }

    return `Valid latin square of size ${m}`
  }
